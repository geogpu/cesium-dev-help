...
// ******************************************************自定义三方库******************************************************
import { exists, copy, mkdirs } from './lib/copy.js'
...
const sourceFiles = [
  coreBasePath + '/core/**/*.ts', //core下所有ts !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
]
...
let createEachPGJs = async (cb) => {
  ...
  fullPaths.forEach((fullPath) => {
    ...
    // 抛出类
    contents +=
      'export { ' + assignmentName + " } from './" + moduleId + "'" + os.EOL
  })
...
/**
 * 测试文件依赖替换
 * 包含package.json 否则import不识别
 */
let copyDist = async () => {
  exists('dist/', 'Tools/test/testPro/node_modules/' + name + '/dist', copy)
  let copyThis = () => fs.copyFileSync('package.json', 'Tools/test/testPro/node_modules/' + name + '/' + 'package.json');
  mkdirs('Tools/test/testPro/node_modules/' + name + '/', copyThis)
...
let GULP_BUILD = series(createEachPGJs, esbuildES6) //生成文件头并打包 mjs ts 复制测试包
let GULP_TEST = series(createEachPGJs, esbuildES6, copyDist) //生成文件头并打包 mjs ts 复制测试包
let GULP_BUILD_NOMINI = series(createEachPGJs, esbuildES6NoMinify) //生成文件头并打包 非压缩js ts
export { GULP_CONNECT, GULP_PACKAGE, GULP_BUILD, GULP_BUILD_NOMINI, GULP_TEST }